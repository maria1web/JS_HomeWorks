let app = {
    ...
    run() {
        ...
        */
    generateBoard() {
        // 3. создаем переменную board и присваиваем ей пустую строку, в ней будет храниться разметка игрового поля в виде строк
        let board = '';
        ...
        for (let i = 0; i < this.config.rows.length; i++) { //из config получаем кол-во строк = длину массива
    ...
    generateRow(startWithColor, rowNum) {
        ...
        for (let i = 0; i < this.config.cols.length; i++) { // строку заполняем колонками
            ...
            if (currentColorClass === 'white') { // то мы начинаем формировать ячейку
                ...
                field = this.generateField('white', rowNum, this.config.cols[i]); // передаём цвет яч, № строки и № колонки
     ...
     */
    generateField(color, rowNum, colChar) {  // принимает цвет яч, № строки и букву столбца
     ...
     */
    insertRowsNumbers() {
        // 7. получаем все теги tr, коллекцию тегов
        let trs = document.querySelectorAll('tr');
        // 7.1 перебираем эти теги в цикле
        for (let i = 0; i <trs.length; i++) {
            // 7.2 внутри тега tr создаем тег td
            ...
            // берём нужный по счету tr и после открытия тега tr
            // вставляем ячейку, тег td с помощью insertAdjacentElement
        }
    },
    ...
    insertPawns() {
        ...
        let whitePawnsRow = document.querySelectorAll('tr:nth-child(7) td'); // = среди тегов tr возьми 7 элемент и дай све его теги td
        ...
        */
    insertFiguresOnDesk() {
        // 5. в цикле перебираем массив positions из файла positions.js
