...
let ticTakToe = {
    gameTableElement: document.getElementById('game'), // свойство с игровым полем, получаем здесь сам тег table
    status: 'playing',
    mapValues: [
        ['', '', ''],
        ['', '', ''],
     ...
     */
    renderMap() {
        for (let row = 0; row < 3; row++) {                 // перебираем строки
            const tr = document.createElement('tr');
            this.gameTableElement.appendChild(tr);          // добавляем в тег-таблицу созданную строку 'tr'
            for (let col = 0; col < 3; col++) {             // перебираем колонки
                let td = document.createElement('td');   // добавляем в созданную строку столбец, формируем ячейки
                td.dataset.row = row.toString();            // передаём координаты ячеек по Х (устанавливаем дада-атрибуты для строк и колонок)
                td.dataset.col = col.toString();            // передаём координаты ячеек по Y (передавая им значения)
                tr.appendChild(td);                         // в нашу строку добавляем ячейку
            }
        }
    },
    ...
    initEventHandlers() {
        ...
        // используем => ф-ю, чтобы метод cellClickHandler захватывал this из окружения, где она написана,
        // т.е. из initEventHandlers, а там this - это объект игры ticTakToe
     ...
     * по ячейке и ячейка, куда был произведен клик, была пустой.
     */
    ...
    isCellEmpty(event) {
        ...
        return this.mapValues[row][col] === ''; // в этом случае вернет true
    },
     ...
     */
    hasWon() {      // определяем координаты выигрышных комбинаций ячеек
     ...
     */
    isLineWon(a, b, c) { // читаем координаты линейных 3-х ячеек
        let value = this.mapValues[a.y][a.x] + this.mapValues[b.y][b.x] + this.mapValues[c.y][c.x]; // проверяем там значения
        return value === 'XXX' || value === '000';     //  проверяем есть ли выиграшная линия?
    },
     ...
     */
    sayWonPhrase() {
        let figure = this.phase === 'X' ? 'Крестики' : 'Нолики'; // если phase === 'X', то figure = 'Крестики', иначе figure = 'Нолики'
     ...
     */
    togglePhase() {
        this.phase = this.phase === 'X' ? '0' : 'X';    // если phase === 'X', то  меняем на this.phase = '0' иначе this.phase = 'X'
    },
